  ...
  return function(app) {
    ...
    Component.parseList = function(components) {
      ...
      if (Array.isArray(components)) {
        ...
        core.dom.find(selector, components || 'body').add(core.dom.filter(selector, components || 'body')).each(function() {
          // Ignore component that has been already started
          if (this.dataset.__sandbox_ref__) {
            return;
          }
          var ns = "aura";
    ...
    return {
      ...
      initialize: function(app) {
        ...
        /**
         * Abstraction to register an associated method callback
         *
         ...
         * @param  {Function} fn            actual function to run
         */
        app.components.register = function(placement, methodName, fn) {
          if (placement !== 'before' && placement !== 'after') {
            throw new Error("Can not register callback on placement type: " + placement);
          }
          var callbackName = placement + ":" + methodName;
          registerComponentCallback(callbackName, fn);
        };

        /**
         * Deregisters an associated method callback
         *
         * @method components.deregister
         * @param  {String}   placement     'before' or 'after'
         * @param  {String}   methodName    eg. 'initialize', 'remove'
         * @param  {Function} fn            actual function to run
         */
        app.components.deregister = function(placement, methodName, fn) {
          if (placement !== 'before' && placement !== 'after') {
            throw new Error("Can not deregister callback on placement type: " + placement);
          }
          var callbackName = placement + ":" + methodName;
          var existingIndex = componentsCallbacks[callbackName].indexOf(fn);
          if (existingIndex === -1) {
            throw new Error("Callback to deregister on " + callbackName + " does not exist");
          }
          deregisterComponentCallback(callbackName,existingIndex);
        };
        ...
        // Actually deregistering the callbacks in the registry.
        function deregisterComponentCallback(callbackName, callbackIndex) {
          componentsCallbacks[callbackName].splice(callbackIndex, 1);
        }
        ...
        var startPromise = app.core.data.deferred().resolve();
        /**
         * Start method.
         ...
         * @return {Promise}                    A promise which will be resolved when components are started
         */
        app.sandbox.start = function (list, options) {
          ...
          startPromise = Component.startAll(list).done(function () {
        ...
        app.sandbox.ready = app.sandbox.readyPromise = function () {
          return startPromise;
        };
      },
