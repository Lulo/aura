define([
  ...
  */
  function Aura(config) {
     ...
     * If a __sandbox_ref__ is provided, that particular sandbox will be stopped
     *
    ...
    app.sandbox.stop = function(selector) {
      ...
      if (sandbox) {
        return stopSandbox(sandbox).done(function() {
      ...
      if (selector) {
        ...
        var _this = this;
        return stopSandbox(this).done(function() {
          cleanUpSandbox(_this);
        });
      }
    };

    function removeSandboxFromArray( sandbox, array ) {
      var index = array.indexOf( sandbox );
      if (index !== -1) {
    ...
    function cleanUpSandbox ( sandbox ) {
      if (app.core.appSandbox._children) {
        removeSandboxFromArray( sandbox, app.core.appSandbox._children );
      }

      if (sandbox._children) {
        removeSandboxFromArray( sandbox, sandbox._children );
      }

      if (sandbox._parent) {
        removeSandboxFromArray( sandbox, sandbox._parent._children );
      }
    }
    ...
    function stopSandbox(sandbox) {
      ...
      if (sandbox) {
        ...
        (sandbox.hasOwnProperty('_children') ? sandbox._children : []).forEach(function (sandbox) {
          sandbox && sandbox.stop();
        });
        app.core.mediator.emit(event, sandbox);
        if (sandbox._component) {
          promise = sandbox._component.invokeWithCallbacks('remove');
          if (typeof sandbox._component.destroy === 'function') {
            sandbox._component.destroy();
          }
        } else {
          promise = app.core.data.deferred().resolve();
        }
        ...
        delete sandbox.el;
        if (sandbox._component) {
          delete sandbox._component.$el;
          if (sandbox._component.options) {
            delete sandbox._component.options.el;
          }
        }
