  ...
  */
  function Aura(config) {
    ...
    app.sandbox.stop = function(selector) {
      ...
      if (sandbox) {
        stopSandbox(sandbox);
        cleanUpSandbox(sandbox.ref, children);
        return;
      }
    ...
    function cleanUpSandbox (ref, children) {
      // Try to remove from children
      var index = _.pluck(children, 'ref').indexOf(ref);
      if (index >= 0) {
        children.splice(index, 1);
        return;
      }

      // Try again with app.core.appSandbox._children
      index = _.pluck(app.core.appSandbox._children || [], 'ref').indexOf(ref);
      if (index >= 0) {
        app.core.appSandbox._children.splice(index, 1);
        return;
      }
    }
